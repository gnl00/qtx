...
if(body && body.hasOwnProperty('data')) {
  ...
  if (url.indexOf("resource/show/tab/v2") !== -1 && method === getMethod) {
    ...
    if (body.data && body.data.hasOwnProperty('top')) {
  ...
  if (body.data && body.data.hasOwnProperty('top_left')) {
    console.log('bili-pure handle 首页 story');
    delete body.data.top_left
  }

    // 首页 tab 栏
    if (body.data && body.data.hasOwnProperty('tab'))  {
    ...
    // 首页 bottom 栏
    if (body.data && body.data.hasOwnProperty('bottom')) {
  ...
  if (url.indexOf("x/resource/top/activity") !== -1 && method === getMethod) {
    console.log('bili-pure handle 首页 top 活动')
    if(body.data && body.data.hasOwnProperty('online')) {
        body.data = {}
    }
  }
}
